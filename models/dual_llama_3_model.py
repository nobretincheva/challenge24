...
class Llama3DualPrompt(Llama3ChatModel):
    ...
    def create_prompt(self, subject_entity: str, relation: str,
        ...
        templates = self.prompt_templates[relation].strip(',')
    ...
    def add_external_info(self, entity_entry, info_strategy):
      ...
      return system_prompt if not system_prompt else "Consider the following additional information: " + system_prompt
    ...
    def clean_output(self, output, prompt):
      ...
      matches = regex.findall(r'final_answer = \[([^\]]*)\]|Final answer = \[([^\]]*)\]', clean_output)
      return matches
    ...
    def use_looping_prompts(self, inp, info_strategy):
      ...
      try:
        ...
        start_count = ast.literal_eval(answer) if inp["Relation"]=='awardWonBy' else 1
        end_count = ast.literal_eval(answer) if inp["Relation"]=='seriesHasNumberOfEpisodes' else 2024
        ...
        for i in range(start_count, end_count + 1):
          ...
          if inp["Relation"]=='seriesHasNumberOfEpisodes':
            try:
              num_ep_per_season = ast.literal_eval(ith_answer[0])
              print(num_ep_per_season)
              final_answer.extend(num_ep_per_season)
              running_sum += num_ep_per_season
            except:
              logger.error(f"Error getting number of episodes for season {i} of " + inp["SubjectEntity"])
          else:
            final_answer.extend(ith_answer)
